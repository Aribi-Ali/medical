...
@section("content")
    <div class="mb-8 text-center">
        <h1 class="text-3xl font-bold text-gray-900">Working Days</h1>
        <p class="mt-2 text-sm text-gray-600">View and manage your working days</p>
    </div>
                        ...
                        <div class="p-6 hover:bg-gray-50 transition-colors">
                            <div class="flex items-center justify-between">
                                <div class="flex items-center space-x-8">
                                    <div class="w-32">
                                        <span class="text-lg font-medium text-gray-900">{{ $workingDay['dayOfWeek'] }}</span>
                                    </div>
                                    <div class="flex items-center space-x-2">
                                        <div class="px-3 py-1 bg-dashboardPrimary-50 text-dashboardPrimary-700 rounded-md">
                                            {{ \Carbon\Carbon::parse($workingDay['startTime'])->format('H:i') }}
                                        </div>
                                        <span class="text-gray-500">to</span>
                                        <div class="px-3 py-1 bg-dashboardPrimary-50 text-dashboardPrimary-700 rounded-md">
                                            {{ \Carbon\Carbon::parse($workingDay['endTime'])->format('H:i') }}
                                        </div>
                                    </div>
                                    <div>
                                </div>
                            ...
                            <div class="flex items-center space-x-4">
                                <form action="{{ route('working-day.destroy') }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this day?');" >
                                    @csrf
                                    <input type="hidden" name="dayOfWeek" value={{ $workingDay['dayOfWeek'] }} />
                                    <button
                                        type="submit"
